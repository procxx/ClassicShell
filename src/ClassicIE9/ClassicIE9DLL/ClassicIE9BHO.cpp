...
HRESULT STDMETHODCALLTYPE CClassicIE9BHO::SetSite( IUnknown *pUnkSite )
{
	...
	IObjectWithSiteImpl<CClassicIE9BHO>::SetSite(pUnkSite);
	if (pUnkSite)
	{
...
void CClassicIE9BHO::ResetParts( void )
{
	int parts[256];
...
STDMETHODIMP CClassicIE9BHO::OnNavigateComplete( IDispatch *pDisp, VARIANT *URL )
{
	HWND status;
	...
	if (text[0])
	{
		...
		HFONT font0=(HFONT)SelectObject(hdc,(HFONT)SendMessage(m_StatusBar,WM_GETFONT,0,0));
		SIZE size;
...
STDMETHODIMP CClassicIE9BHO::OnQuit( void )
{
